...
import { entities } from '@/app/lib/definitions';

export interface CreateRecordFormInterface<T> {
    dependencies: (typeof entities[number]['name'])[]; // array of entities the current entity depends on
    sampleRecord: T;
}

export default async function CreateRecordForm<T>({
    dependencies,
    sampleRecord,
}: CreateRecordFormInterface<T>) {

    const _dependencies = entities
        // filter the entities that are listed in dependencies
        .filter(entity => dependencies.includes(entity.name));

    const dependenciesData = await Promise.all(
        _dependencies.map(entity => entity.fetchCallback()));
