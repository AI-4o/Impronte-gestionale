...
import { ServizioATerraInputGroup, VoloInputGroup } from "@/app/dashboard/(overview)/general-interface-create/general-interface.defs";
    ...
    if (serviziATerra.rowCount > 0) {
      for (let i = 0; i < serviziATerra.rowCount; i++) {
        ...
        const iG = new ServizioATerraInputGroup(
          ...
          serviziATerra.rows[i].ricarico,
          serviziATerra.rows[i].servizio_aggiuntivo,
          serviziATerra.rows[i].tot,
